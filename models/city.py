...
class CommunityInfo(Base):
	...
	ershoufang_source_count = Column(types.String(32), nullable=False)#在售二手房数量
	...
	def __init__(self, city_id, info):
		...
		if info.__contains__("basic_info"):
			...
			if basic_info.__contains__("ershoufang_source_count"):
				self.ershoufang_source_count = basic_info["ershoufang_source_count"]
...
class HistoryRecord(Base):
	...
	__table_args__ = (PrimaryKeyConstraint('city_id', 'house_code','sign_timestamp'),)
	city_id = Column(types.Integer)
